export const series = [
  {
    title: "Jujutsu Kaisen",
    seriesId: "jujutsukaisen",
    thumbnail:
      "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692263659/satoru-gojo_vtykvf.jpg",
    desc: "Jujutsu Kaisen is a Japanese manga series written and illustrated by Gege Akutami. It has been serialized in Shueisha&apos;s shōnen mangamagazine Weekly Shōnen Jump since March 2018, with its chapterscollected and published in 23 tankōbon volumes as of July 2023.",
    year: 2020,
    language: 2,
    subtitle: 8,
    seasons: [
      {
        season: 1,
        episodes: [
          {
            episode: 1,
            title: "Episode 1",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692263659/satoru-gojo_vtykvf.jpg",
          },
          {
            episode: 2,
            title: "Episode 2",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692263659/satoru-gojo_vtykvf.jpg",
          },
        ],
        thumbnail:
          "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692263659/satoru-gojo_vtykvf.jpg",
      },
      {
        season: 2,
        episodes: [
          {
            episode: 1,
            title: "Episode 1",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692263659/satoru-gojo_vtykvf.jpg",
          },
        ],
        thumbnail:
          "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692263659/satoru-gojo_vtykvf.jpg",
      },
    ],
    imdb: 8.3,
    genre: ["comedy"],
    cast: ["Will Arnett"],
  },
  {
    title: "Bojack Horseman",
    seriesId: "bojackhorseman",
    thumbnail:
      "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692279386/bojack_zookqd.jpg",
    desc: "A humanoid horse, BoJack Horseman -- lost in a sea of self-loathing and booze -- decides it's time for a comeback. Once the star of a '90s sitcom, in which he was the adoptive father of three orphaned kids (two girls and a boy).",
    year: 2014,
    language: 2,
    subtitle: 6,
    seasons: [
      {
        season: 1,
        episodes: [
          {
            episode: 1,
            title: "Episode 1",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692279386/bojack_zookqd.jpg",
          },
          {
            episode: 2,
            title: "Episode 2",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692279386/bojack_zookqd.jpg",
          },
        ],
        thumbnail:
          "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692279386/bojack_zookqd.jpg",
      },
      {
        season: 2,
        episodes: [
          {
            episode: 1,
            title: "Episode 1",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692279386/bojack_zookqd.jpg",
          },
        ],
        thumbnail:
          "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692279386/bojack_zookqd.jpg",
      },
    ],
    imdb: 8.3,
    genre: ["comedy"],
    cast: ["Will Arnett"],
  },
  {
    title: "Itaewon Class",
    seriesId: "itaewonclass",
    thumbnail:
      "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692267734/itaewon-class_vqb2ep.jpg",
    desc: "An ex-con and his friends fight to make their ambitious dreams for their street bar a reality.",
    year: 2020,
    language: 2,
    subtitle: 6,
    seasons: [
      {
        season: 1,
        episodes: [
          {
            episode: 1,
            title: "Episode 1",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692267734/itaewon-class_vqb2ep.jpg",
          },
          {
            episode: 2,
            title: "Episode 2",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692267734/itaewon-class_vqb2ep.jpg",
          },
        ],
        thumbnail:
          "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692267734/itaewon-class_vqb2ep.jpg",
      },
    ],
    imdb: 8.3,
    genre: ["comedy"],
    cast: ["Will Arnett"],
  },
  {
    title: "It's Okay To Not Be Okay",
    seriesId: "iotnbo",
    thumbnail:
      "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266342/iotnbo_aseyzv.jpg",
    desc: "A road to emotional healing opens up for an antisocial children's book author and an employee in a psychiatric hospital.",
    year: 2020,
    language: 2,
    subtitle: 6,
    seasons: [
      {
        season: 1,
        episodes: [
          {
            episode: 1,
            title: "Episode 1",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266342/iotnbo_aseyzv.jpg",
          },
          {
            episode: 2,
            title: "Episode 2",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266342/iotnbo_aseyzv.jpg",
          },
        ],
        thumbnail:
          "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266342/iotnbo_aseyzv.jpg",
      },
    ],
    imdb: 8.3,
    genre: ["comedy"],
    cast: ["Will Arnett"],
  },
  {
    title: "Start-Up",
    seriesId: "startup",
    thumbnail:
      "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266261/start-up_nqgb2n.jpg",
    desc: "Seo Dal Mi has dreams of becoming Korea's own Steve Jobs, and with her genius first love, an investor, and a business insider by her side, her dream may be closer than she thinks.",
    year: 2020,
    language: 2,
    subtitle: 6,
    seasons: [
      {
        season: 1,
        episodes: [
          {
            episode: 1,
            title: "Episode 1",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266261/start-up_nqgb2n.jpg",
          },
          {
            episode: 2,
            title: "Episode 2",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266261/start-up_nqgb2n.jpg",
          },
        ],
        thumbnail:
          "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266261/start-up_nqgb2n.jpg",
      },
    ],
    imdb: 8.3,
    genre: ["comedy"],
    cast: ["Will Arnett"],
  },
  {
    title: "Crash Landing On You",
    seriesId: "crashlanding",
    thumbnail:
      "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266198/clou_gntccr.jpg",
    desc: "A paragliding mishap drops a South Korean heiress in North Korea - and into the life of an army officer, who decides he will help her hide.",
    year: 2019,
    language: 2,
    subtitle: 6,
    seasons: [
      {
        season: 1,
        episodes: [
          {
            episode: 1,
            title: "Episode 1",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266198/clou_gntccr.jpg",
          },
          {
            episode: 2,
            title: "Episode 2",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266198/clou_gntccr.jpg",
          },
        ],
        thumbnail:
          "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266198/clou_gntccr.jpg",
      },
    ],
    imdb: 8.3,
    genre: ["comedy"],
    cast: ["Will Arnett"],
  },
  {
    title: "Vincenzo",
    seriesId: "vincenzo",
    thumbnail:
      "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266168/vincenzo_vrpdio.jpg",
    desc: "During a visit to his motherland, a Korean-Italian mafia lawyer gives a conglomerate a taste of its own medicine with a side of justice.",
    year: 2021,
    language: 2,
    subtitle: 6,
    seasons: [
      {
        season: 1,
        episodes: [
          {
            episode: 1,
            title: "Episode 1",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266168/vincenzo_vrpdio.jpg",
          },
          {
            episode: 2,
            title: "Episode 2",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266168/vincenzo_vrpdio.jpg",
          },
        ],
        thumbnail:
          "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692266168/vincenzo_vrpdio.jpg",
      },
    ],
    imdb: 8.3,
    genre: ["comedy"],
    cast: ["Will Arnett"],
  },
  {
    title: "Rent A Girlfriend",
    seriesId: "rentagirlfriend",
    thumbnail:
      "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692264341/rent-a-girlfriend_my393j.png",
    desc: "Rent-A-Girlfriend is an anime series adapted from the manga series of the same title written by Reiji Miyajima.",
    year: 2017,
    language: 2,
    subtitle: 6,
    seasons: [
      {
        season: 1,
        episodes: [
          {
            episode: 1,
            title: "Episode 1",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692264341/rent-a-girlfriend_my393j.png",
          },
          {
            episode: 2,
            title: "Episode 2",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692264341/rent-a-girlfriend_my393j.png",
          },
        ],
        thumbnail:
          "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692264341/rent-a-girlfriend_my393j.png",
      },
      {
        season: 2,
        episodes: [
          {
            episode: 1,
            title: "Episode 1",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692264341/rent-a-girlfriend_my393j.png",
          },
          {
            episode: 2,
            title: "Episode 2",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692264341/rent-a-girlfriend_my393j.png",
          },
        ],
        thumbnail:
          "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692357061/rent-a-girl-2_s2ajqe.jpg",
      },
      {
        season: 3,
        episodes: [
          {
            episode: 1,
            title: "Episode 1",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692264341/rent-a-girlfriend_my393j.png",
          },
          {
            episode: 2,
            title: "Episode 2",
            thumbnail:
              "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692264341/rent-a-girlfriend_my393j.png",
          },
        ],
        thumbnail:
          "https://res.cloudinary.com/doa2rj1yw/image/upload/v1692357182/rent-a-girl-3_zjlxba.jpg",
      },
    ],
    imdb: 8.3,
    genre: ["comedy"],
    cast: ["Will Arnett"],
  },
];
